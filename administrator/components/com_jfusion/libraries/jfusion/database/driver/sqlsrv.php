...
class JDatabaseDriverSqlsrv extends JDatabaseDriver
{
	 ...
	 *
	 * @return  Driver_Sqlsrv  Returns this object to support chaining.
	 *
	 * @since   12.1
	 */
	...
	public function execute()
	{
		...
		if (!is_resource($this->connection))
		{
			JLog::add(Text::sprintf('JLIB_DATABASE_QUERY_FAILED', $this->errorNum, $this->errorMsg), JLog::ERROR, 'database');
		...
		if (!$this->cursor)
		{
			...
			if (!$this->connected())
			{
				...
				catch (RuntimeException $e)
				{
					...
					JLog::add(Text::sprintf('JLIB_DATABASE_QUERY_FAILED', $this->errorNum, $this->errorMsg), JLog::ERROR, 'databasequery');
					throw new RuntimeException($this->errorMsg, $this->errorNum);
				}
