...
if ( !class_exists('DokuWikiSearch') ) {
    class DokuWikiSearch {
         ...
         * @return array
         */
        public static function ft_pageSearch($query, &$highlight) {
            $q = self::ft_queryParser($query);
            $highlight = array();
            ...
            // combine and-words
            if (count($q['and']) > 1) {
                $docs = self::ft_resultCombine($q['and']);
            } else {
                $docs = $q['and'][0];
            }
         ...
         * @return array
         */
        public static function ft_backlinks($id) {
            global $conf;
            ...
            $page = noNS($id);
            $matches = self::idx_lookup(self::idx_tokenizer($page, $stopwords)); // pagename may contain specials (_ or .)
            $docs = array_keys(self::ft_resultCombine(array_values($matches)));
         ...
         * @return array
         */
        public static function ft_mediause($id, $max) {
            global $conf;
            $swfile = DOKU_INC . 'inc/lang/' . $conf['lang'] . '/stopwords.txt';
            $stopwords = @file_exists($swfile) ? file($swfile) : array();
            ...
            $media = noNS($id);
            $matches = self::idx_lookup(self::idx_tokenizer($media, $stopwords));
            $docs = array_keys(self::ft_resultCombine(array_values($matches)));
            if (!count($docs)) return $result;
         ...
         * @return array
         */
        public static function ft_pageLookup($id, $pageonly = true) {
         ...
         * @return string
         */
        public static function ft_snippet($id, $highlight) {
            $text = rawWiki($id);
            $match = array();
            $snippets = array();
         ...
         * @return array
         */
        public static function ft_resultCombine($args) {
         ...
         * @return array
         */
        public static function ft_queryParser($query) {
            global $conf;
            $swfile = DOKU_INC . 'inc/lang/' . $conf['lang'] . '/stopwords.txt';
            ...
            while (preg_match('/"(.*?)"/', $query, $match)) {
                $q['phrases'][] = $match[1];
                $q['and'] = array_merge($q['and'], self::idx_tokenizer($match[0], $stopwords));
                $query = preg_replace('/"(.*?)"/', '', $query, 1);
            }
            $words = explode(' ', $query);
            foreach ($words as $w) {
                if ($w{0} == '-') {
                    $token = self::idx_tokenizer($w, $stopwords, true);
         ...
         * @return array
         */
        public static function idx_tokenizer($string, &$stopwords, $wc = false) {
            $words = array();
            $wc = ($wc) ? '' : $wc = '\*';
            if (preg_match('/[^0-9A-Za-z]/u', $string)) {
                ...
                $arr = explode(' ', self::utf8_stripspecials($string, ' ', '\._\-:' . $wc));
                foreach ($arr as $w) {
         ...
         * @return array
         */
        public static function idx_lookup($words) {
            global $conf;
            $result = array();
            $wids = self::idx_getIndexWordsSorted($words, $result);
            if (empty($wids)) return array();
            // load known words and documents
            $page_idx = self::idx_getIndex('page', '');
            $docs = array(); // hold docs found
            foreach (array_keys($wids) as $wlen) {
                $wids[$wlen] = array_unique($wids[$wlen]);
                $index = self::idx_getIndex('i', $wlen);
                foreach ($wids[$wlen] as $ixid) {
                    if ($ixid < count($index)) $docs["$wlen*$ixid"] = self::idx_parseIndexLine($page_idx, $index[$ixid]);
                }
            }
         ...
         * @return array
         */
        public static function idx_getIndexWordsSorted($words, &$result) {
            ...
            $indexes_known = self::idx_indexLengths($length_filter);
            if (!empty($tokenwild)) sort($indexes_known);
            // get word IDs
            $wids = array();
            foreach ($indexes_known as $ixlen) {
                $word_idx = self::idx_getIndex('w', $ixlen);
                // handle exact search
         ...
         * @return array
         */
        public static function idx_indexLengths(&$filter) {
            global $conf, $rootFolder;
         ...
         * @return array
         */
        public static function idx_getIndex($pre, $wlen) {
            global $conf, $rootFolder;
         ...
         * @return array
         */
        public static function idx_parseIndexLine(&$page_idx, $line) {
            $result = array();
            $line = trim($line);
            ...
            foreach ($parts as $part) {
                ...
                if (!self::page_exists($doc, '', false)) continue;
                $result[$doc] = $cnt;
            }
            return $result;
        }
         ...
         * @return bool
         */
        public static function page_exists($id, $rev = '', $clean = true) {
            return @file_exists(self::wikiFN($id, $rev, $clean));
        }
         ...
         * @return string
         */
        public static function wikiFN($raw_id, $rev = '', $clean = true) {
            global $conf, $rootFolder;
            global $cache_wikifn;
            $cache = & $cache_wikifn;
            ...
            $id = str_replace(':', '/', $id);
            if (empty($rev)) {
                $fn = $rootFolder . '/data/pages' . '/' . self::utf8_encodeFN($id) . '.txt';
            } else {
                $fn = $conf['olddir'] . '/' . self::utf8_encodeFN($id) . '.' . $rev . '.txt';
                if ($conf['compression']) {
         ...
         * @return mixed|string
         */
        public static function utf8_encodeFN($file, $safe = true) {
         ...
         * @return bool
         */
        public static function isHiddenPage($id) {
            global $conf;
