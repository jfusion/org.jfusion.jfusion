    ...
    function loadGallery2Api($fullInit, $itemId = null) {
        if (!$this->loadedGallery) {
            ...
            if (!is_file($index_file)) {
                JFusionFunction::raiseWarning('The path to the Gallery2('.$this->getJname().' path: '.$index_file.') embed file set in the component preferences does not exist');
            } else {
                ...
                if ($ret) {
                    JFusionFunction::raiseWarning('Error while initialising Gallery2 API');
                } else {
                    $ret = GalleryCoreApi::setPluginParameter('module', 'core', 'cookie.path', '/');
                    if ($ret) {
                        JFusionFunction::raiseWarning('Error while setting cookie path');
                    } else {
                        if ($fullInit) {
