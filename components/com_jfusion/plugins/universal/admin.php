          ...
          foreach ($mapuser as $key => $val) {
            ...
            if ( isset($value['value'][$val->Field]) ) {
              $fieldsvalue = $value['value'][$val->Field];
            	if (is_array($fieldsvalue)) {
					foreach ($fieldsvalue as $key => $val2) {
						$fieldsvalue[$key] = htmlentities($val2);
					}
            	} else {
            ...
            switch ($fieldstype) {
                ...
                case 'VALUE':
              	...
              	case 'ONOFF':
					foreach ($fieldsvalue as $key => $val2) {
	                	$output .= '<input type="text" id="'.$control_name.$name.$type.'value'.$val->Field.$key.'" name="'.$control_name.'['.$name.']['.$type.'][value]['.$val->Field.']['.$key.']" value="'.$val2.'" size="40" class="inputbox" />';
					}
                    break;
            }
            $output .= '</div>';
